...
"""
        ...
        result = runner.invoke(cli, [
            ...
            '--editor', 'claude',
            '--output', str(temp_dir),
            '-V', 'PROJECT_TITLE=OverriddenProject',
            '-V', 'AUTHOR_EMAIL=override@test.com',
            '-V', 'PROJECT_TYPE=mobile app',
            '-V', 'TECH_STACK=React Native',
            str(var_file)
        ])
...
"""
        ...
        result = runner.invoke(cli, [
            'generate',
            '--editor', 'claude',
            '--output', str(temp_dir / 'claude_test'),
        ...
        result = runner.invoke(cli, [
            'generate',
            '--editor', 'continue',
            '--output', str(temp_dir / 'continue_test'),
            str(conditional_file)
        ])
        ...
        assert 'Continue-specific: Generate comprehensive completions' in continue_content
        ...
        result = runner.invoke(cli, [
            'generate',
            '--editor', 'codeium',
            '--output', str(temp_dir / 'codeium_test'),
            str(conditional_file)
        ])
        ...
        general_guidelines = [g for g in codeium_data['guidelines'] if g['category'] == 'general']
...
"""
        ...
        result = runner.invoke(cli, [
            ...
            '--editor', 'claude',
            '--output', str(temp_dir),
            '-V', 'PROJECT_NAME=OverriddenAdvanced',
            '-V', 'AUTHOR=Override Tester',
            '-V', 'MAIN_APPROACH=agile',
