...
class TestCursorAdapterSpecInclusion:
    ...
    def test_v3_1_generates_spec_files(self, adapter, spec_dir):
        ...
        # Check that at least one spec file was created (Cursor uses .mdc extension)
        spec_files = list((spec_dir / ".cursor/rules").glob("spec-*.mdc"))
    ...
    def test_v3_1_disabled_no_spec_files(self, adapter, spec_dir):
        ...
        adapter.generate(prompt, spec_dir, dry_run=False)
        # Check that no spec files were created (Cursor uses .mdc extension)
        if (spec_dir / ".cursor/rules").exists():
            spec_files = list((spec_dir / ".cursor/rules").glob("spec-*.mdc"))
            assert len(spec_files) == 0
