...
from src.apm.core.models import UniversalPrompt, PromptMetadata, Condition
...
class TestConditionalProcessor:
    ...
    def test_evaluate_condition_in_list(self, processor):
        ...
        assert processor._evaluate_condition("EDITOR in [\"claude\", \"cursor\"]", variables) is True
        assert processor._evaluate_condition("EDITOR in [\"copilot\", \"continue\"]", variables) is False
        assert processor._evaluate_condition("LANG in [\"python\", \"javascript\"]", variables) is True
            ...
            conditions=[
                Condition(
                    **{"if": "EDITOR == \"claude\""},
                    then={
                        "instructions": {
                            "general": ["Claude-specific instruction"]
                        }
                    }
                )
            ]
