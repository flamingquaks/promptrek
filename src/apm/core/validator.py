...
class UPFValidator:
    ...
    # Supported editors
    KNOWN_EDITORS = {
        'copilot', 'cursor', 'continue', 'claude', 'kiro', 
        'cline', 'codeium', 'tabnine', 'amazon-q', 'jetbrains'
    }
    ...
    def _validate_schema_version(self, prompt: UniversalPrompt, result: ValidationResult) -> None:
        """Validate schema version."""
        version = prompt.schema_version
        
        # Check if it's a supported version
        if version != "1.0.0":
            result.add_warning(f"Schema version '{version}' may not be fully supported. Current supported version: 1.0.0")
    
    def _validate_targets(self, prompt: UniversalPrompt, result: ValidationResult) -> None:
        ...
        if unknown_editors:
            result.add_warning(f"Unknown target editors: {', '.join(sorted(unknown_editors))}")
        
        # Check for duplicates
        if len(targets) != len(set(targets)):
            result.add_warning("Duplicate target editors found")
    
    def _validate_metadata(self, prompt: UniversalPrompt, result: ValidationResult) -> None:
        ...
        if not version or not self._is_valid_semver(version):
            result.add_warning(f"Metadata version '{version}' is not a valid semantic version")
    
    def _validate_context(self, prompt: UniversalPrompt, result: ValidationResult) -> None:
        ...
        # Check project type
        if context.project_type and context.project_type not in self.KNOWN_PROJECT_TYPES:
    ...
    def _validate_instructions(self, prompt: UniversalPrompt, result: ValidationResult) -> None:
        ...
        for field_name in ['general', 'code_style', 'architecture', 'testing', 'security', 'performance']:
    ...
    def _validate_examples(self, prompt: UniversalPrompt, result: ValidationResult) -> None:
    ...
    def _validate_variables(self, prompt: UniversalPrompt, result: ValidationResult) -> None:
        ...
        invalid_names = [key for key in variables.keys() if not self._is_valid_variable_name(key)]
        if invalid_names:
            result.add_warning(f"Invalid variable names (should be UPPER_SNAKE_CASE): {', '.join(invalid_names)}")
    
    def _validate_editor_specific(self, prompt: UniversalPrompt, result: ValidationResult) -> None:
        ...
        if unknown_configs:
            result.add_warning(f"Editor-specific configs for non-target editors: {', '.join(unknown_configs)}")
    
    def _validate_conditions(self, prompt: UniversalPrompt, result: ValidationResult) -> None:
    ...
    def _validate_imports(self, prompt: UniversalPrompt, result: ValidationResult) -> None:
        ...
        for i, import_config in enumerate(prompt.imports):
            ...
            if not path.endswith(('.apm.yaml', '.apm.yml')):
                result.add_warning(f"Import {i+1} path should end with .apm.yaml or .apm.yml: {path}")
    ...
    def _is_valid_variable_name(self, name: str) -> bool:
        ...
        return (name.isupper() and 
                name.replace('_', '').replace('0', '').replace('1', '').replace('2', '').replace('3', '').replace('4', '').replace('5', '').replace('6', '').replace('7', '').replace('8', '').replace('9', '').isalpha() and
                name[0].isalpha())