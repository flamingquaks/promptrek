...
from .exceptions import UPFParsingError, UPFFileNotFoundError
...
class UPFParser:
    ...
    def parse_file(self, file_path: Union[str, Path]) -> UniversalPrompt:
        ...
        if not file_path.suffix in ['.yaml', '.yml']:
            raise UPFParsingError(f"File must have .yaml or .yml extension: {file_path}")
    ...
    def parse_dict(self, data: Dict[str, Any], source: str = "<dict>") -> UniversalPrompt:
        ...
        if not isinstance(data, dict):
            raise UPFParsingError(f"UPF data must be a dictionary, got {type(data)} in {source}")
    ...
    def parse_string(self, yaml_content: str, source: str = "<string>") -> UniversalPrompt:
        """
    ...
    def find_upf_files(self, directory: Union[str, Path], recursive: bool = False) -> list[Path]:
        """
