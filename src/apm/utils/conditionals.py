...
from ..core.models import UniversalPrompt, Condition
...
class ConditionalProcessor:
    ...
    def process_conditions(self, prompt: UniversalPrompt, 
    ...
    def _evaluate_condition(self, condition: str, variables: Dict[str, Any]) -> bool:
        ...
        if " in " in condition:
            ...
            if right.strip().startswith('[') and right.strip().endswith(']'):
                values = [v.strip().strip('"\'') for v in right.strip()[1:-1].split(',')]
