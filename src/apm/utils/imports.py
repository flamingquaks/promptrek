...
from ..core.models import UniversalPrompt, ImportConfig
...
class ImportProcessor:
    ...
    def process_imports(self, prompt: UniversalPrompt, base_path: Path) -> UniversalPrompt:
        """
        ...
        for import_config in prompt.imports:
            ...
            merged_data = self._merge_imported_data(merged_data, imported_data, import_config.prefix)
    ...
    def _process_single_import(self, import_config: ImportConfig, base_path: Path) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if imported_prompt.imports:
                imported_prompt = self.process_imports(imported_prompt, import_path.parent)
    ...
    def _merge_imported_data(self, base_data: Dict[str, Any], imported_data: Dict[str, Any], 
    ...
    def _merge_instructions(self, base_data: Dict[str, Any], imported_instructions: Dict[str, Any], 
        ...
        for category, instructions in imported_instructions.items():
            if isinstance(instructions, list):
                ...
                for instruction in instructions:
                    prefixed_instruction = f"[{prefix}] {instruction}" if prefix else instruction
                    base_instructions[category].append(prefixed_instruction)
    
    def _merge_examples(self, base_data: Dict[str, Any], imported_examples: Dict[str, Any], 
                       prefix: Optional[str]) -> None:
    ...
    def _merge_variables(self, base_data: Dict[str, Any], imported_variables: Dict[str, Any], 
                        prefix: Optional[str]) -> None:
