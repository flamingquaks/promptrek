...
from ...core.models import UniversalPrompt, PromptMetadata
...
def init_command(ctx: click.Context, template: Optional[str], output: str) -> None:
    ...
    try:
        with open(output_path, 'w', encoding='utf-8') as f:
            yaml.dump(upf_data, f, default_flow_style=False, allow_unicode=True, sort_keys=False)
...
def _get_template(template_name: str) -> dict:
    ...
    if template_name not in templates:
        ...
        raise CLIError(f"Unknown template '{template_name}'. Available templates: {available}")
...
def _get_react_template() -> dict:
    ...
    template['metadata']['description'] = 'AI assistant for React TypeScript development'
    ...
    template['examples']['component'] = '''```typescript
...
def _get_api_template() -> dict:
    ...
    template['context']['technologies'] = ['python', 'fastapi', 'postgresql', 'sqlalchemy']
    template['instructions']['general'].extend([
    ...
    template['examples']['endpoint'] = '''```python
