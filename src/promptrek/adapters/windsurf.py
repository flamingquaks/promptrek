...
class WindsurfAdapter(MCPGenerationMixin, MarkdownSyncMixin, EditorAdapter):
    ...
    def _generate_v2(
        ...
        if headless:
            bootstrap_file = self._generate_bootstrap_file(
                prompt, rules_dir, created_files, dry_run, verbose
            )
            if bootstrap_file:
    ...
    def parse_files(
        ...
        # All other .md files in .windsurf/rules/ become documents
        documents = []
        main_content = None
        ...
        # Use default content if no general.md was found
        if main_content is None:
            main_content = "# Windsurf AI Assistant\n\nNo general.md found."
